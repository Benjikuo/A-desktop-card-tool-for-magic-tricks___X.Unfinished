...
if __name__ == "__main__":
    ...
    case_img = load_image(os.path.join(IMAGE_FOLDER, "case.png"), BOX_SIZE)
    ...
    card_box = CardBox(
        canvas, BOX_POSITION[0], BOX_POSITION[1], case_img, back_img, card_imgs
    )
    ...
    # ✅ Ctrl + D：刪除所有卡片（帶星星特效）
    def clear_all_with_effect(event=None):
        global focused_card
    ...
    root.bind("<Control-F>", flip_all)
    root.bind("<Control-d>", clear_all_with_effect)
    root.bind("<Control-D>", clear_all_with_effect)
