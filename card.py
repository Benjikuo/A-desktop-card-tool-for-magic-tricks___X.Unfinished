...
class Box(Drag):
    ...
    def delete_card(self, targets, n=None):
        if n is None:
            n = len(targets)
        ...
        def delete_next(i):
            if i < len(targets):
                if targets[i] in self.used_card:
                    targets[i].delete(count=max(3, int(10 - n // 7)))
    ...
    def spawn_spread(self, event=None, group="all", sort="standard", delete_used=True):
        if self.spreading:
            return
